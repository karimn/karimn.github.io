...
Principal, Zahrcast | Economist \& Bayesian Statistician\\[1mm]
karimn2.0@pm.me \quad karimn.co \quad zahrcast.com \quad github.com/karimn
\end{center}
...
Economist and applied Bayesian statistician with 10+ years building \textbf{hierarchical models} for \textbf{longitudinal and survival data}. Runs Zahrcast, which forecasts oncology trial outcomes from immature interim data. Previously at AstraZeneca, building \textbf{patient-level digital-twin models} of oncology tumor dynamics with real-world evidence integration; earlier work spans field experiments in East Africa and South Asia, real-estate market design, and health-behavior interventions.
...
\textbf{Principal} | \textit{Zahrcast LLC} \hfill Jul 2026 -- present
\begin{itemize}
  \item Bayesian forecasting for oncology trials in flight. Joint models of tumor dynamics and multistate survival forecast what a trial will report at maturity from immature interim data, partially pooling historical trials, to support go/no-go and other interim decisions
\end{itemize}
...
\begin{itemize}
  ...
  \item On No Lean Season, contributed to the analysis that failed to replicate the earlier pilot's effect on migration and consumption; the finding informed Evidence Action's decision to close the program
...
\textbf{Impact.} Predictive-accuracy curves by forecast horizon for both biomarker and survival outcomes. These show how well the model forecasts more directly than aggregate LOO does, and let Phase 3 decision reviews check forecast calibration.

\subsection*{Agentic-workflow layer for Bayesian statistical modeling}

\textbf{Problem.} Bayesian trial-modeling work has two hard parts that off-the-shelf AI assistants don't address. The first is the mathematics: multistate survival with joint longitudinal submodels, non-centered hierarchical parameterizations, competing risks, and interval censoring produce dense derivations that are slow and error-prone for one person working by hand. The second is the development loop: MCMC runs take hours to days, diagnostics and reparameterization require domain judgment, and state across the feedback loop is hard to carry.

\textbf{Method.} Designed and built an agentic-workflow layer on Claude Code, shared across the modeling team via an internal plugin marketplace. Core components: collaborative mathematical derivation, where Claude works through the statistical mathematics with the modeler before it becomes Stan code; a persistent memory system for carrying model-specific context across sessions; domain-specific sub-agents (Stan diagnostic triage, prior sensitivity review, \texttt{targets} pipeline explorer); MCP-integrated skills for pipeline operations and job management on Domino; and reusable skill and plugin templates that codify team conventions for Bayesian workflow. Version-controlled and iterated like any other team codebase.

\textbf{Impact.} Derivations are checked collaboratively and documented, instead of done by one person by hand. The time between an MCMC run finishing and deciding what to do next is shorter; that step accounts for most of the delay in Bayesian model development. Adopted across the modeling team for reproducible Stan development, MCMC diagnostics triage, and pipeline operations.
